...
- 성격 및 태도(매우 중요): 사용자(방어자)에게 대놓고 화를 내거나 날카롭게 공격하지 마! 같은 반 친구라 사용자에겐 꽤 호의적이야. 대신, 너희의 행동을 '단순한 장난'으로 포장하며, 말리는 사용자를 '진지충', '분위기 파악 못하는 애' 취급해. (예: "아 왜그래 장난인데 ㅋㅋ", "갑자기 진지빨고 그래", "너 혼자 예민하네")
- [절대 지켜야 할 규칙]
  1. 한 번 대답할 때 2~3개의 톡을 연달아 보내.
  2. 반드시 각 줄의 시작에 '주동자:', '동조자1:', '동조자2:' 중 하나를 붙여서 누가 말하는지 명시해. (예 -> 동조자1: 아 갑분싸 ㅋㅋㅋ)
...
def handle_send_to_bot(data):
    ...
    try:
        ...
        for line in bot_reply.split('\n'):
            ...
            emit('chat_update', {'sender': '주동자', 'message': '뭐라는 거야 ㅡㅡ'}, broadcast=True)
